...
Watershed
```{r}
klamath_basin <- rivermile::klamath_hucs |> st_union()
...
```{r}
layers <- st_layers(here::here("data-raw", "shapes", "nhdplus_epasnapshot2022_or.gdb"))
...
catchment_fabric_or <- st_read(here::here("data-raw", "shapes", "nhdplus_epasnapshot2022_or.gdb"), layer =  "catchment_fabric_or")

```

```{r}
nhd_catchment_ids <- rivermile::processed_nhd |> 
  rename(nhd_plus_id = nhd_pl_) |> 
  pull(nhd_plus_id) |> unique()
# just for Oregon...
catchments <- catchment_fabric_or |> filter(nhdplusid %in% nhd_catchment_ids)

```

```{r}
layers <- st_layers(here::here("data-raw", "shapes", "18", "NHDPLUS_H_1801_HU4_GDB.gdb"))

catchment_ca <- st_read(here::here("data-raw", "shapes", "18", "NHDPLUS_H_1801_HU4_GDB.gdb"), layer =  "NHDPlusCatchment")
